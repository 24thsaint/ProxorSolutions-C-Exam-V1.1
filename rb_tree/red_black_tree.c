...
rb_red_blk_tree* RBTreeCreate( int (*CompFunc) (const void*,const void*),
  ...
  /*  for information on nil and root_parent */
  temp=newTree->nil= (rb_red_blk_node*) SafeMalloc(sizeof(rb_red_blk_node));
  temp->parent=temp->left=temp->right=temp;
  temp->red=0;
  temp->key=0;
  temp=newTree->root_parent= (rb_red_blk_node*) SafeMalloc(sizeof(rb_red_blk_node));
...
/*           root_parent and nil pointers, and the node to rotate on. */
/**/
/*  OUTPUT:  None */
/**/
...
void LeftRotate(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  /* count on the root_parent sentinel to implicitly take care of this case */
  if( x == x->parent->left) {
    x->parent->left=y;
  } else {
    x->parent->right=y;
...
rb_red_blk_node * RBTreeInsert(rb_red_blk_tree* tree, void* key, void* info) {
  ...
  tree->root_parent->left->red=0;
  return(newNode);

#ifdef DEBUG_ASSERT
  Assert(!tree->nil->red,"nil not red in RBTreeInsert");
  Assert(!tree->root_parent->red,"root_parent not red in RBTreeInsert");
#endif
}
...
rb_red_blk_node* TreeSuccessor(rb_red_blk_tree* tree,rb_red_blk_node* x) { 
  rb_red_blk_node* y;
  rb_red_blk_node* nil=tree->nil;
  rb_red_blk_node* root_parent=tree->root_parent;
...
void InorderTreePrint(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  ...
  if (x != tree->nil) {
    ...
    printf("  p->key=");
    if( x->parent == root_parent) printf("NULL"); else tree->PrintKey(x->parent->key);
    printf("  red=%i\n",x->red);
...
void RBTreeDestroy(rb_red_blk_tree* tree) {
  TreeDestHelper(tree,tree->root_parent->left);
  free(tree->root_parent);
  free(tree->nil);
  free(tree);
}
...
void RBTreePrint(rb_red_blk_tree* tree) {
  InorderTreePrint(tree, tree->root_parent->left);
}
...
/**/
/*    INPUTS:  tree is the tree in question and q is a pointer to the key */
...
rb_red_blk_node* RBExactQuery(rb_red_blk_tree* tree, void* q) {
  rb_red_blk_node* x=tree->root_parent->left;
  rb_red_blk_node* nil=tree->nil;
  int compVal;
  if (x == nil) return(0);
...
void RBDeleteFixUp(rb_red_blk_tree* tree, rb_red_blk_node* x) {
  rb_red_blk_node* root_parent=tree->root_parent->left;
  rb_red_blk_node* w;

  while( (!x->red) && (root_parent != x)) {
    if (x == x->parent->left) {
      w=x->parent->right;
      if (w->red) {
	w->red=0;
	...
	w->right->red=0;
	LeftRotate(tree,x->parent);
	x=root_parent; /* this is to exit while loop */
      }
...
void RBDelete(rb_red_blk_tree* tree, rb_red_blk_node* z){
  ...
  if (root_parent == (x->parent = y->parent)) { /* assignment of y->p to x->p is intentional */
    root_parent->left=x;
  } else {
    if (y == y->parent->left) {
      y->parent->left=x;
...
/**/
/*    OUTPUT:  list containing pointers to the nodes between [low,high] */
/**/
/*    Modifies Input: none */
/***********************************************************************/

lst_list* RBEnumerate(rb_red_blk_tree* tree, void* low, void* high) {
  lst_list* enumResultList;
  rb_red_blk_node* nil=tree->nil;
  rb_red_blk_node* x=tree->root_parent->left;
  rb_red_blk_node* lastBest=nil;

  enumResultList=ListCreate();
  while(nil != x) {
  ...
  while ( (lastBest != nil) && (1 != tree->Compare(low,lastBest->key))) {
    ListPush(enumResultList,lastBest);
...
/**/
/*    OUTPUT:  This function returns the node with the lowest key or */
/*             nil if no node exists. */
/**/
/*    Modifies Input: none */
/**/
/***********************************************************************/
  
rb_red_blk_node *RBLowest(rb_red_blk_tree* tree)
{
  rb_red_blk_node* x = tree->root_parent->left;
  rb_red_blk_node* nil = tree->nil;
  rb_red_blk_node* low = nil;
  while (nil != x) {
    low = x;
    x = x->left;
  }
...
/*    INPUTS:  tree is the tree in question. */
/**/
/*    OUTPUT:  This function returns the node with the highest key or */
/*             nil if no node exists. */
/**/
/*    Modifies Input: none */
/**/
...
rb_red_blk_node *RBHighest(rb_red_blk_tree* tree)
{
  rb_red_blk_node* x = tree->root_parent->left;
  rb_red_blk_node* nil = tree->nil;
  rb_red_blk_node* high = nil;
  while (nil != x) {
    high = x;
    x = x->right;
  }
...
/*    INPUTS:  tree is the tree in question. */
/**/
/*    OUTPUT:  This function returns the root node of the tree or */
/*             NULL if no root exists. */
/**/
/*    Modifies Input: none */
/**/
/***********************************************************************/

rb_red_blk_node *RBRoot(rb_red_blk_tree* tree)
{
  rb_red_blk_node *root = tree->root_parent->left;
